...
export function CoinNewsFeed({
    ...
    async function refresh() {
      try {
        const res = await fetch(
          `/api/coin-news?id=${encodeURIComponent(coinId)}&_=${Date.now()}`,
          { cache: "no-store" },
        );
        if (!res.ok) return;
  ...
  return (
    <section aria-labelledby="coin-news-feed" className="rounded-xl border border-white/10 bg-[#101217] p-3">
      <div className="flex items-center justify-between gap-2">
        <h2 id="coin-news-feed" className="text-sm font-semibold text-zinc-100">
          In the news
        </h2>
        {stale ? (
      ...
      <p className="mt-1 text-[11px] text-zinc-500">
        Newest first · times in US Eastern (EST/EDT)
      </p>
      <div className="mt-3 space-y-2">
        {hasNewsItems ? (
          items.map((item) => (
            <article key={item.id} className="rounded-lg border border-white/10 bg-[#0d0f14] px-2.5 py-2">
              ...
              <a
                ...
                className="line-clamp-2 text-xs font-semibold text-zinc-100 underline-offset-2 hover:text-[#a855f7] hover:underline"
              >
          ...
          <div className="rounded-lg border border-white/10 bg-[#0d0f14] px-2.5 py-2">
            <p className="text-[11px] text-zinc-400">No recent articles found yet for this coin query.</p>
          </div>
        )}
      </div>
      {sourceUrl ? (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="mt-3 inline-flex text-[11px] font-medium text-[#a855f7] underline-offset-2 hover:underline"
        >
          Open full news feed
        </a>
      ) : null}
